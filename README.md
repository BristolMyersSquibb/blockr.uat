...
- `dependencies.R` — scan-only declarations so deploy-optional `Suggests` the app never names in its own code (e.g. `ids`) still reach the generated manifest.
- `DESCRIPTION` — runtime dependencies (`Imports:` / `Suggests:`), with every blockr.* package pinned to its GitHub main branch (`Remotes:`) so the deploy resolves development versions rather than stale CRAN/release builds.
