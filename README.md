...
- `DESCRIPTION` — runtime dependencies (`Imports:`, including `ids` so the friendly board IDs blockr.core generates when it's present are available on Connect), with every blockr.* package pinned to its GitHub main branch (`Remotes:`) so the deploy resolves development versions rather than stale CRAN/release builds.
